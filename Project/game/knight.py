...
class Knight(Enemy):
    ...
    def __init__(self, assets_path):
        ...
        self.image = pygame.image.load(os.path.join(assets_path, "k0.png"))
        self.rect = self.image.get_rect()
        self.pos = vec(WIDTH, 200)
        self.vel = vec(0, 0)
        self.acc = vec(0, 0)
        self.hp = 50
    ...
    def jump(self, player, enemy):
        ...
        if enemy.death == True:
            if abs((player.pos.x - 50) - self.pos.x) <= 200 and self.jmpcooldown == False:
            ...
            if self.is_jumping == True and self.crush == False:
                ...
                self.acc.y = 0.05
                if int(abs((player.pos.x - 50) - self.pos.x)) <= 3:
                    self.crush = True
            if self.crush == True:
